"""binary_clf_evaluation.py: Defines class for evaluating performance of binary classifiers"""
import json
import logging
import os
...
from .binary_clf_plotting import _plot_roc_curve, _make_roc_dict, _plot_lift_chart, _make_lift_dict, \
...
class BinaryModelInsights:
    ...
    def __init__(self, model_factory: BinaryModelFactory, num_importances: int = 10, importance_sample_size: int = 500):
    ...
    def generate_report(self, model_paths: ModelPaths) -> str:
        """
    ...
    def _calc_counts(self, X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.Series, y_test: pd.Series):
        # X_train and X_test included for future count statistics
