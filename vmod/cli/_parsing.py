"""_parsing.py: Tools for adding common options to CLI parsers"""
import argparse
import json
import logging
import re
...
def parser_add_collector_options(parser, base=True, chunk=True, label=True, balance=True):
    ...
    if base:
        ...
        parser.add_argument('--required-fields', nargs='*', action='store', type=str, default=[],
                            help='Columns that are required to be present when reading in the data. This helps catch errors early on before'
                                 ' appending or modeling data. Takes multiple arguments.')
    ...
    if balance:
        parser.add_argument('--balance-fields', nargs='*', action='store', type=str, default=[],
                            help='Columns to perform missing value balancing on. Columns are only balanced when using the `train` subcommand,'
                                 ' and only if the --label is provided. Not compatible with --chunksize option. Takes multiple arguments.')
